document.addEventListener("DOMContentLoaded", () => {
  ...
  function toggleTheme() {
    ...
    if (document.body.classList.contains("dark-theme")) {
      ...
      themeToggleMobile.innerHTML = '<i class="fas fa-sun"></i><span>Tema Chiaro</span>'
      localStorage.setItem("theme", "dark")
    } else {
      themeToggle.innerHTML = '<i class="fas fa-moon"></i>'
      themeToggleMobile.innerHTML = '<i class="fas fa-moon"></i><span>Tema Scuro</span>'
      ...
      .forEach((categoryName, index) => {
        ...
        categoryCard.innerHTML = `
          <div class="category-card-content">
            ...
            <h3>${displayName}</h3>
            <p>${categories[categoryName].length} ${categories[categoryName].length === 1 ? "Progetto" : "Progetti"}</p>
          </div>
        `
