class ColorCraft {
    constructor() {
        ...
        this.trashedPalettes = JSON.parse(localStorage.getItem("trashedPalettes")) || []
    ...
    bindEvents() {
        ...
        this.colorFormat.addEventListener("change", () => this.updatePaletteDisplay())
        ...
        // Navigation events
        document.getElementById("savedPalettesBtn").addEventListener("click", () => this.showSavedPalettes())
        document.getElementById("trashedPalettesBtn").addEventListener("click", () => this.showTrashedPalettes())

        // Modal events
        document.getElementById("closeSavedModal").addEventListener("click", () => this.hideModal(this.savedPalettesModal))
        document
            .getElementById("closeTrashedModal")
            .addEventListener("click", () => this.hideModal(this.trashedPalettesModal))
        document.getElementById("closeExportModal").addEventListener("click", () => this.hideModal(this.exportModal))
        document.getElementById("closeImageModal").addEventListener("click", () => this.hideModal(this.imageModal))

        // Export events
        document.querySelectorAll(".export-option").forEach((btn) => {
            btn.addEventListener("click", (e) => this.selectExportFormat(e.target.closest(".export-option")))
        })
        ...
        document
            ...
            ;[this.savedPalettesModal, this.trashedPalettesModal, this.exportModal, this.imageModal].forEach((modal) => {
    ...
    generateColorHarmony(baseColor, harmonyType, count, brightness, saturation) {
        ...
        switch (harmonyType) {
            case "monochromatic":
                colors.push(...this.generateMonochromatic(baseHsl, count, brightness, saturation))
                break
            case "analogous":
                colors.push(...this.generateAnalogous(baseHsl, count, brightness, saturation))
                break
            case "complementary":
                colors.push(...this.generateComplementary(baseHsl, count, brightness, saturation))
                break
            case "triadic":
                colors.push(...this.generateTriadic(baseHsl, count, brightness, saturation))
                break
            case "tetradic":
                colors.push(...this.generateTetradic(baseHsl, count, brightness, saturation))
                break
            case "splitComplementary":
                colors.push(...this.generateSplitComplementary(baseHsl, count, brightness, saturation))
                break
            case "compound":
                colors.push(...this.generateCompound(baseHsl, count, brightness, saturation))
                break
        }
    ...
    generateMonochromatic(baseHsl, count, brightness, saturation) {
        ...
        for (let i = 0; i < count; i++) {
            const newL = Math.max(10, Math.min(90, l + (i - Math.floor(count / 2)) * (brightness / count)))
            const newS = Math.max(20, Math.min(100, s + (Math.random() - 0.5) * (saturation / 100) * 20))
    ...
    generateAnalogous(baseHsl, count, brightness, saturation) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            const newL = Math.max(10, Math.min(90, l + (Math.random() - 0.5) * (brightness / 100) * 40))
            const newS = Math.max(20, Math.min(100, s + (Math.random() - 0.5) * (saturation / 100) * 30))
    ...
    deletePermanently(id) {
        if (!confirm("Sei sicuro di voler eliminare definitivamente questa palette?")) return
    ...
    emptyTrash() {
        ...
        if (!confirm("Sei sicuro di voler svuotare completamente il cestino?")) return
    ...
    updateExportCode() {
        const activeFormat = document.querySelector(".export-option.active").dataset.format
        let code = ""
    ...
    generatePaletteImage() {
        ...
        this.currentPalette.forEach((color, index) => {
            ...
            variations.forEach((variation, varIndex) => {
                ...
                ctx.fillRect(x, mainHeight + varIndex * varHeight, colorWidth, varHeight)
            })
        })
    ...
    updateStorage() {
        ...
        localStorage.setItem("trashedPalettes", JSON.stringify(this.trashedPalettes))
    }
