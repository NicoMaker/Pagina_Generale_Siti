document.addEventListener("DOMContentLoaded", () => {
  ...
  const currentPhaseDescription = document.getElementById(
  ...
  const phaseDescriptions = {
    ...
    "waxing-gibbous":
      "Più della metà della Luna è visibile e continua a crescere verso la Luna piena.",
    full: "La Luna è completamente illuminata vista dalla Terra, poiché si trova sul lato opposto della Terra rispetto al Sole.",
    "waning-gibbous":
      "La Luna inizia a diminuire la sua illuminazione, ma più della metà è ancora visibile.",
    "last-quarter":
  ...
  function getMoonPhase(date) {
    ...
    const hour =
      date.getHours() + date.getMinutes() / 60.0 + date.getSeconds() / 3600.0;
  ...
  function updateMoonImage(phase, position) {
    ...
    switch (phase) {
      ...
      case "waxing-crescent":
        ...
        break;
      case "first-quarter":
        currentMoonImage.style.background =
          "linear-gradient(90deg, #121620 50%, var(--moon-color) 50%)";
      ...
      case "last-quarter":
        currentMoonImage.style.background =
          "linear-gradient(270deg, #121620 50%, var(--moon-color) 50%)";
        currentMoonImage.style.boxShadow = "0 0 15px rgba(255, 255, 255, 0.4)";
        break;
      ...
      if (
        ...
        for (let hour = 0; hour < 24; hour++) {
          ...
          if (phase === "new") phaseMatch = Math.abs(hourMoonData.position - 0);
            ...
            phaseMatch = Math.abs(hourMoonData.position - 0.25);
          else if (phase === "full")
            phaseMatch = Math.abs(hourMoonData.position - 0.5);
          else if (phase === "last-quarter")
            phaseMatch = Math.abs(hourMoonData.position - 0.75);
  ...
  function calculateAllMoonPhasesForYear(year) {
    ...
    const allMainPhaseDates = [
      ...
      ...mainPhases["first-quarter"].map((date) => ({
        date,
        phase: "first-quarter",
      })),
      ...mainPhases.full.map((date) => ({ date, phase: "full" })),
      ...mainPhases["last-quarter"].map((date) => ({
        date,
        phase: "last-quarter",
      })),
    ...
    for (let i = 0; i < allMainPhaseDates.length - 1; i++) {
      ...
      if (currentPhase.phase === "new" && nextPhase.phase === "first-quarter") {
        ...
        currentPhase.phase === "first-quarter" &&
        nextPhase.phase === "full"
      ) {
        intermediatePhase = "waxing-gibbous";
      } else if (
        currentPhase.phase === "full" &&
        nextPhase.phase === "last-quarter"
      ) {
        intermediatePhase = "waning-gibbous";
      } else if (
        currentPhase.phase === "last-quarter" &&
        nextPhase.phase === "new"
      ) {
      ...
      const midDate = new Date(
        (currentPhase.date.getTime() + nextPhase.date.getTime()) / 2,
      );
    ...
    if (allMainPhaseDates.length > 0) {
      ...
      if (
        allMainPhaseDates[allMainPhaseDates.length - 1].phase === "last-quarter"
      ) {
  ...
  function calculateMoonPhasesForMonth(year, month) {
    ...
    for (const phase in yearPhases) {
      monthPhases[phase] = yearPhases[phase].filter(
  ...
  function displayMoonPhasesForYear(year) {
    ...
    for (const phase in phases) {
      ...
      if (phases[phase].length === 0) {
        datesContainer.innerHTML =
          '<div class="loading">Nessuna data disponibile</div>';
        continue;
      }
  ...
  function generateMonthlyCalendar(year, month) {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      const moonIndicator = createMoonPhaseIndicator(
        moonPhaseToday.phase,
        moonPhaseToday.position,
      );
