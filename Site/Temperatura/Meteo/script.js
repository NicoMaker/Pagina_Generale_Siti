...
async function searchWeather(city) {
  ...
  try {
    ...
    if (city.toLowerCase() === "roma" || city.toLowerCase() === "rome") {
      ...
      italianCities.some((c) => c.toLowerCase() === city.toLowerCase())
    ) {
...
async function fetchWeatherData(city) {
  const response = await fetch(
    `${BASE_URL}/weather?q=${city}&units=metric&lang=it&appid=${API_KEY}`,
  );
  ...
  if (!response.ok) {
    if (response.status === 404) {
      ...
      throw new Error(
        "Errore nel caricamento dei dati meteo. Riprova più tardi.",
      );
    }
  }
...
async function fetchAirQualityData(lat, lon) {
  try {
    ...
    console.error(
      "Errore nel caricamento dei dati sulla qualità dell'aria:",
      error,
    );
...
function displayWeatherData(weatherData, forecastData, airQualityData) {
  ...
  if (
    isItalianCity ||
    weatherData.name === "Roma" ||
    weatherData.name === "Rome"
  ) {
    countryCode = "IT";
  }
