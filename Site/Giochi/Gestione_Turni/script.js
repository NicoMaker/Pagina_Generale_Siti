...
function createAddPersonInTurnSection() {
  ...
  turnSection
    ...
    .insertBefore(addPersonContainer, peopleContainer);
...
function renderPeopleList() {
      ...
      if (isGameStarted) {
        if (index === currentPersonIndex) {
          ...
          completeBtn.innerHTML =
            '<i class="fas fa-check-circle"></i> <span>Completato</span>';
  ...
  if (isGameStarted && people.length < 2) {
    showNotification(
      "Servono almeno 2 persone per continuare! Aggiungi un'altra persona.",
    );
  }
}
...
function showNotification(
  message = "Ordine aggiornato! Tocca ora alla prima persona",
) {
...
function startTurns() {
  ...
  // Mostra notifica
  showNotification(
    `Turno iniziato! Tocca a ${people[currentPersonIndex].name}`,
  );
}
...
function completeTurn() {
  ...
  if (currentPersonIndex >= people.length) {
    ...
    if (allCompleted) {
      ...
      showNotification(
        `Turno ${currentTurn} iniziato! Tocca a ${people[currentPersonIndex].name}`,
      );
    } else {
      ...
      // Mostra notifica
      showNotification(
        `${currentPersonName} ha completato! Tocca a ${people[currentPersonIndex].name}`,
      );
    }
  } else {
    // Mostra notifica
