...
class DualWheelOfFortune {
  ...
  generateAuto(wheelNumber) {
    ...
    if (wheelNumber === 1) {
      ...
      notificationSystem.show(`${this.wheel1Names.length} nomi generati automaticamente!`, "success")
    } else {
      ...
      notificationSystem.show(`${this.wheel2Names.length} nazioni generate automaticamente!`, "success")
    }
  ...
  loadFromStorage() {
    try {
      ...
      notificationSystem.show("Errore nel caricamento dei dati salvati", "error")
    }
  }
  ...
  async addItem(wheelNumber) {
    ...
    if (items.includes(inputValue)) {
      notificationSystem.show(`Questo ${itemType} è già presente nella lista`, "warning")
      return
    }
  ...
  async editName(index, wheelNumber) {
    ...
    const newItem = await modalSystem.prompt(`Modifica ${itemType}:`, currentItem, `Modifica ${itemType}`)
    ...
    const wheelName = wheelNumber === 1 ? "nomi" : "nazioni"
    notificationSystem.show(`${itemType} modificato nella ruota ${wheelName}: "${newItem}"`, "success")
  }
  ...
  spinWheel(wheelNumber) {
    ...
    if (items.length === 0) {
      const itemType = wheelNumber === 1 ? "nomi" : "nazioni"
      notificationSystem.show(`Aggiungi almeno un ${itemType.slice(0, -1)} prima di girare la ruota!`, "warning")
      return
    }
    ...
    const wheelName = wheelNumber === 1 ? "nomi" : "nazioni"
    notificationSystem.show(`La ruota ${wheelName} sta girando... 🎲`, "info", "In corso", 4000)
    ...
    setTimeout(() => {
      ...
      notificationSystem.show(`🎉 Il ${resultType} estratto è: ${winner}!`, "success", "Abbiamo un vincitore!", 8000)
    }, 4000)
  }
    ...
    reader.onload = async (e) => {
      try {
        ...
        if (itemsToAdd.length === 0) {
          const itemType = wheelNumber === 1 ? "nomi" : "nazioni"
          notificationSystem.show(`Tutti i ${itemType} nel file sono già presenti nella lista`, "info")
          return
        }
        ...
        notificationSystem.show("Errore nel caricamento del file. Controlla il formato.", "error")
      }
    }
  ...
  exportTxt(wheelNumber) {
    ...
    if (items.length === 0) {
      const itemType = wheelNumber === 1 ? "nomi" : "nazioni"
      notificationSystem.show(`Nessun ${itemType.slice(0, -1)} da esportare!`, "warning")
      return
    }
    ...
    try {
      ...
      notificationSystem.show("Errore durante l'esportazione del file TXT", "error")
    }
  }
  ...
  exportJson(wheelNumber) {
    ...
    try {
      ...
      notificationSystem.show("Errore durante l'esportazione del file JSON", "error")
    }
  }
