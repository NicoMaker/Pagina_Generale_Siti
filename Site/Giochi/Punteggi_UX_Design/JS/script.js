...
const deleteParticipantName = document.getElementById(
  "delete-participant-name",
);
const deleteParticipantStats = document.getElementById(
...
function setupEventListeners() {
  ...
  if (confirmDeleteBtn) {
    confirmDeleteBtn.addEventListener(
      "click",
      confermaEliminazionePartecipante,
    );
  }
...
function mostraDeleteParticipantModal(index) {
  ...
  announceForScreenReader(
    `Conferma eliminazione di ${partecipante.nome} ID ${partecipante.id}`,
  );
}
...
function confermaEliminazionePartecipante() {
  if (participantToDelete >= 0 && participantToDelete < partecipanti.length) {
    ...
    showToast(
      `${partecipante.nome} #${partecipante.id} eliminato definitivamente`,
      "success",
    );
  }
}
...
function aggiungiPartecipante() {
  ...
  salvaDati();
  console.log(
    "Partecipanti dopo aggiunta:",
    JSON.parse(JSON.stringify(partecipanti)),
  );
...
const participantsCheckboxList = document.getElementById(
...
function aggiornaSelezionePartecipante() {
  ...
  const allChecked = participantsCheckboxList.querySelector(
    'input[type="checkbox"].all-checkbox',
  )?.checked;
  participantsCheckboxList.innerHTML = "";
  const searchTerm = participantSearchInput
    ? participantSearchInput.value.trim().toLowerCase()
    : "";
  // Opzione "Tutti"
  ...
  partecipantiOrdinati.forEach((partecipante) => {
    if (searchTerm && !partecipante.nome.toLowerCase().includes(searchTerm))
      return;
    ...
    checkbox.addEventListener("change", function () {
      if (checkbox.checked) {
        if (!selectedParticipantIds.includes(partecipante.id))
          selectedParticipantIds.push(partecipante.id);
      } else {
        selectedParticipantIds = selectedParticipantIds.filter(
          (id) => id !== partecipante.id,
        );
      }
  ...
  allCheckbox.addEventListener("change", function () {
    participantDivs.forEach(({ checkbox, div }) => {
      ...
      if (allCheckbox.checked) {
        if (!selectedParticipantIds.includes(Number(checkbox.value)))
          selectedParticipantIds.push(Number(checkbox.value));
      } else {
        selectedParticipantIds = [];
      }
    });
    console.log(
      "selectedParticipantIds dopo click su 'Tutti':",
      selectedParticipantIds,
    );
    updateHighlightAndAllCheckbox();
  });
  ...
  if (allChecked) {
    ...
    console.log(
      "selectedParticipantIds dopo ripristino 'Tutti':",
      selectedParticipantIds,
    );
  }
  updateHighlightAndAllCheckbox();
...
function getSelectedParticipantIndexes() {
  return selectedParticipantIds
    .map((id) => partecipanti.findIndex((p) => p.id === id))
    .filter((idx) => idx !== -1);
}
...
function aggiungiPunti() {
  ...
  if (selectedIndexes.length === 0) {
    const debugIds =
      selectedParticipantIds.length > 0
        ? ` (ID selezionati: ${selectedParticipantIds.join(", ")})`
        : "";
  ...
  showToast(
    `${punti} punti aggiunti a ${selectedIndexes.length} partecipante/i`,
    "success",
  );
  aggiornaListaPartecipanti();
  pointsInput.value = "";
  ...
  if (participantsCheckboxList) {
    const checkboxes = participantsCheckboxList.querySelectorAll(
      'input[type="checkbox"]',
    );
    ...
    const items = participantsCheckboxList.querySelectorAll(
      ".participant-checkbox-item, .all-checkbox",
    );
...
function togliPunti() {
  ...
  showToast(
    `${punti} punti tolti a ${selectedIndexes.length} partecipante/i`,
    "success",
  );
  aggiornaListaPartecipanti();
  pointsInput.value = "";
...
`;
  ...
  const fileFeedbackCancelBtn = document.getElementById(
    "file-feedback-cancel-btn",
  );
    ...
    reader.onload = (e) => {
      try {
        ...
        if (fileExtension === "json") {
          ...
          importData.modalitaVittoria =
            typeof jsonData.modalitaVittoria === "string"
              ? jsonData.modalitaVittoria
              : null;
          results.total = Array.isArray(jsonData.partecipanti)
            ? jsonData.partecipanti.length
            : 0;
            ...
            jsonData.partecipanti.forEach((item) => {
              try {
                ...
                if (
                  ...
                  results.errors.push({
                    message: `Nome mancante o non valido: ${JSON.stringify(item)}`,
                  });
                  return;
                }
                ...
                if (isNaN(punti)) {
                  results.errors.push({
                    message: `Punti non validi per ${nome}: ${item.punti}`,
                  });
                  return;
                }
          ...
          if (righe.length > 0 && righe[0].startsWith("#MODALITA_VITTORIA:")) {
            ...
            if (modeMatch && modeMatch[1]) {
              ...
              results.warnings.push({
                message: `Formato modalità vittoria non valido nella prima riga: ${modeLine}`,
              });
            }
            startIndex = 1;
          }
          ...
          for (let i = startIndex; i < righe.length; i++) {
            ...
            if (sepIndex === -1) {
              results.errors.push({
                message: `Riga non valida (manca ':'): ${riga}`,
              });
              continue;
            }
            ...
            if (nomeLower === "null" || nomeLower === "#null") {
              results.errors.push({
                message: `Nome non valido (null o #null): ${nome}`,
              });
              continue;
            }
            if (isNaN(punti)) {
              results.errors.push({
                message: `Punti non validi per ${nome}: ${puntiStr}`,
              });
              continue;
            }
            importData.partecipanti.push({ nome, punti });
        ...
        showFileFeedbackModal(
          ...
          function () {
            ...
            const nomiEsistenti = new Set(
              partecipanti.map((p) => p.nome.trim().toLowerCase()),
            );
            ...
            pendingImport.partecipanti.forEach((item) => {
              ...
              if (!nome || typeof nome !== "string" || isNaN(punti)) {
                console.warn(
                  "Tentativo di importare partecipante non valido:",
                  item,
                );
                return;
              }
            ...
            selectedParticipantIds = [];
            console.log(
              "Partecipanti dopo import:",
              JSON.parse(JSON.stringify(partecipanti)),
            );
            if (
              pendingImport.modalitaVittoria &&
              (pendingImport.modalitaVittoria === "max" ||
                pendingImport.modalitaVittoria === "min")
            ) {
          ...
          function () {
            ...
            showToast(
              "Caricamento annullato, nessun dato importato",
              "warning",
            );
            pendingImport = null;
          },
          {
            ...
            warningMessage:
              "ATTENZIONE: Caricando un file, i partecipanti nuovi verranno aggiunti a quelli già presenti. Quelli già esistenti non verranno modificati. Procedere?",
          },
        );
      } catch (error) {
...
function aggiornaSelezionePartecipante() {
  ...
  const filtro =
    document.getElementById("participant-search")?.value.toLowerCase() || "";
  ...
  if (searchInput) {
    searchInput.addEventListener("input", () => {
      const tuttiSelezionati =
        selectedParticipantIds.length === partecipanti.length;
      if (!tuttiSelezionati) {
        aggiornaSelezionePartecipante();
      }
    });
...
function aggiornaSelezionePartecipante() {
  ...
  const tuttiFiltratiSelezionati = partecipantiFiltrati.every((p) =>
  ...
  allCheckbox.addEventListener("change", () => {
    if (allCheckbox.checked) {
      ...
      selectedParticipantIds = selectedParticipantIds.filter(
        (id) => !partecipantiFiltrati.some((p) => p.id === id),
      );
    }
    aggiornaSelezionePartecipante();
  });
