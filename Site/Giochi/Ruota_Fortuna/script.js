...
class WheelOfFortune {
  ...
  async editName(index) {
    if (this.isSpinning) {
      notifications.show("Non puoi modificare i nomi mentre la ruota gira", "warning")
      return
    }

    const currentName = this.names[index]
    const newName = await modal.prompt("Modifica nome:", currentName, "Modifica Nome")
  ...
  async deleteName(index) {
    if (this.isSpinning) {
      notifications.show("Non puoi eliminare partecipanti mentre la ruota sta girando", "warning")
      return
    }
  ...
  async clearAll() {
    if (this.isSpinning) {
      notifications.show("Attendi che la ruota finisca di girare prima di resettare", "warning")
      return
    }
  ...
  spinWheel() {
    if (this.names.length === 0) {
      notifications.show("Aggiungi almeno un nome prima di girare la ruota!", "warning")
      return
    }
    ...
    setTimeout(() => {
      ...
      notifications.show(`🎉 Il vincitore è: ${winner}!`, "success", "Abbiamo un vincitore!", 8000)
    }, 4000)
  }
  ...
  hideResult = () => document.getElementById("resultDisplay").classList.remove("show")
    ...
    reader.onload = async (e) => {
      try {
        ...
        if (namesToAdd.length === 0) {
          notifications.show("Tutti i nomi nel file sono già presenti nella lista", "info")
          return
        }
  ...
  exportJson() {
    ...
    try {
      ...
      notifications.show("Errore durante l'esportazione del file JSON", "error")
    }
  }
}
