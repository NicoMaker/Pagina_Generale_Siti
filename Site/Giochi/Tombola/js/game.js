...
function startGame() {
  ...
  if (isFirstStart) {
    ...
    isFirstStart = false
    gameStarted = true
    extractBtn.innerHTML = '<i class="fas fa-random"></i> <span>Estrai Numero</span>'
...
function toggleAutoGenerate() {
  if (autoGenerateInterval) {
    ...
    autoBtn.innerHTML = '<i class="fas fa-pause"></i> <span>Ferma Automatico</span>'
  ...
  for (let i = 0; i < 100; i++) {
    ...
    const animation = confetti.animate(
      [
        ...
        {
          transform: `translate(${Math.random() * 50 - 25}px, ${window.innerHeight
