...
function scorePassword(pw) {
  ...
  if (score >= 6) return { score, label: 'Forte', cls: 'health-strong', tier: 2 };
  if (score >= 4) return { score, label: 'Media', cls: 'health-medium', tier: 1 };
...
function addEntry() {
  ...
  if (!site || !pass) { showToast('⚠ Sito e password sono obbligatori'); return; }
...
function copyPassword(pw) {
  navigator.clipboard.writeText(pw).then(() => showToast('✓ Password copiata!'));
}

function renderVault() {
  const list = document.getElementById('vault-list');
  document.getElementById('count-label').textContent = `${vault.length} ${vault.length === 1 ? 'voce' : 'voci'}`;
  ...
  list.innerHTML = vault.map(entry => {
...
function updateDashboard() {
  ...
  if (total === 0) { grade = '–'; desc = 'Aggiungi password'; color = 'var(--text-dim)'; }
  else if (score >= 80) { grade = 'OTTIMO'; desc = 'Vault ben protetto'; color = 'var(--green)'; ring.style.stroke = 'var(--green)'; }
  else if (score >= 60) { grade = 'BUONO'; desc = 'Qualche miglioramento possibile'; color = 'var(--green)'; ring.style.stroke = 'var(--green)'; }
  else if (score >= 40) { grade = 'DISCRETO'; desc = 'Rafforza le password deboli'; color = 'var(--yellow)'; ring.style.stroke = 'var(--yellow)'; }
  else { grade = 'CRITICO'; desc = 'Vault a rischio elevato'; color = 'var(--red)'; ring.style.stroke = 'var(--red)'; }
  ...
  if (dups.length === 0) {
    dupList.innerHTML = '<div class="no-dup">✓ Nessuna password duplicata rilevata</div>';
  } else {
    dupList.innerHTML = '<div class="dup-list">' +
      dups.map(sites => `
        <div class="dup-item">
