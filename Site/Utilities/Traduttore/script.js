document.addEventListener("DOMContentLoaded", function () {
  ...
  const sourceLanguageDisplay = document.getElementById(
    "source-language-display",
  );
  const targetLanguageDisplay = document.getElementById(
    "target-language-display",
  );
  const sourceLanguageDropdown = document.getElementById(
    "source-language-dropdown",
  );
  const targetLanguageDropdown = document.getElementById(
  ...
  const sourceLanguageSearch = document.getElementById(
    "source-language-search",
  );
  const targetLanguageSearch = document.getElementById(
  ...
  const sourceLanguageSelector = document.getElementById(
  ...
  function populateLanguageList(listElement, searchInput, isSource = true) {
    ...
    if (isSource) {
      const autoDetectIndex = filteredLanguages.findIndex(
        (lang) => lang.code === "auto",
      );
    ...
    filteredLanguages.forEach((lang) => {
      ...
      if (
        (isSource && lang.code === currentSourceLang) ||
  ...
  function swapLanguages() {
    ...
    if (currentSourceLang === "auto") {
      showNotification(
        'Non è possibile scambiare quando la lingua di origine è "Rileva lingua"',
      );
      return;
    }
  ...
  function toggleFullscreen() {
    if (!document.fullscreenElement) {
      document.documentElement.requestFullscreen().catch((err) => {
        showNotification(
          `Errore nella modalità a schermo intero: ${err.message}`,
          true,
        );
      });
    } else {
  ...
  function updateHistoryDisplay() {
    ...
    if (translationHistory.length === 0) {
      historyList.innerHTML =
        '<div class="history-empty">Nessuna traduzione recente</div>';
      return;
    }
  ...
  sourceLanguageDisplay.addEventListener("click", () => {
    sourceLanguageDropdown.style.display =
      sourceLanguageDropdown.style.display === "block" ? "none" : "block";
    targetLanguageDropdown.style.display = "none";
  });

  targetLanguageDisplay.addEventListener("click", () => {
    targetLanguageDropdown.style.display =
      targetLanguageDropdown.style.display === "block" ? "none" : "block";
    sourceLanguageDropdown.style.display = "none";
  });
