...
const calcolaPersonalizzataBtn = document.getElementById(
  "calcolaPersonalizzataBtn",
);
const annullaPersonalizzataBtn = document.getElementById(
  "annullaPersonalizzataBtn",
);
...
const radioPersonalizzata = document.querySelector(
  'input[value="personalizzata"]',
);

radioUguali.addEventListener("change", () => {
  modalitaScelta = "uguali";
});
radioPersonalizzata.addEventListener("change", () => {
  modalitaScelta = "personalizzata";
});
...
function calcolaPersonalizzata() {
  ...
  if (sommaPercentuali > 100.01) {
    mostraAlert(
      `La somma delle percentuali è ${sommaPercentuali.toFixed(1)}%, non può superare 100%!`,
    );
    return;
  }
  ...
  if (
    personeSenzaPercentuale === 0 &&
    Math.abs(sommaPercentuali - 100) > 0.01
  ) {
    mostraAlert(
      `Hai inserito percentuali per tutti ma la somma è ${sommaPercentuali}%, deve essere 100%!`,
    );
    return;
  }
  ...
  for (let i = 0; i < persone.length; i++) {
    ...
    if (percentualiInserite[i] !== null) {
      ...
      percentualeUsata =
        personeSenzaPercentuale > 0
          ? percentualeRestante / personeSenzaPercentuale
          : 0;
    }
