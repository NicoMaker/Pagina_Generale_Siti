...
function renderNode(node, name, depth, filter, isRoot = false, nodePath = []) {
  ...
  const selCount = folderFiles.filter((f) =>
    fileInlineSelection.has(f.fullPath),
  ).length;
  folderChk.textContent =
    selCount === folderFiles.length && folderFiles.length > 0
      ? "✓"
      : selCount > 0
        ? "–"
        : "";
  folderChk.classList.toggle(
    "fchk-all",
    selCount === folderFiles.length && folderFiles.length > 0,
  );
  folderChk.classList.toggle(
    "fchk-some",
    selCount > 0 && selCount < folderFiles.length,
  );
  folderChk.title = "Seleziona/deseleziona tutti i file in questa cartella";
  folderChk.addEventListener("click", (e) => {
    e.stopPropagation();
    const allSel = folderFiles.every((f) =>
      fileInlineSelection.has(f.fullPath),
    );
    folderFiles.forEach((f) =>
      allSel
        ? fileInlineSelection.delete(f.fullPath)
        : fileInlineSelection.add(f.fullPath),
    );
    updateFileSelBar();
    renderTree();
  });
...
function renderFile(file) {
  ...
  chk.addEventListener("click", (e) => {
    e.stopPropagation();
    if (fileInlineSelection.has(file.fullPath))
      fileInlineSelection.delete(file.fullPath);
    else fileInlineSelection.add(file.fullPath);
    div.classList.toggle(
      "file-item-selected",
      fileInlineSelection.has(file.fullPath),
    );
...
async function scaricaFileSelezionatiInline() {
  if (!fileInlineSelection.size) {
    showToast("⚠ Nessun file selezionato");
    return;
  }

  const allFiles = collectAllFiles();
  const sel = allFiles.filter((f) =>
    fileInlineSelection.has(f.file?.fullPath || f.path),
  );
