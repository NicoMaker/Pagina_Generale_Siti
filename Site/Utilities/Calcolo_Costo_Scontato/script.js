...
class DiscountCalculator {
    ...
    init() {
        ...
        this.elements = {
            ...
            calculatePercentageBtn: document.getElementById('calculate-percentage-btn'),
    ...
    bindEvents() {
        ...
        this.elements.originalPriceInput.addEventListener('input', () => this.updateLivePreview());
        this.elements.originalPriceInput.addEventListener('blur', () => this.validatePrice(this.elements.originalPriceInput, this.elements.priceError));
        
        // Corrected update for discount input to also update slider
        this.elements.discountInput.addEventListener('input', (e) => this.updateDiscountFromInput(e.target.value));
        this.elements.discountInput.addEventListener('blur', () => this.validateDiscount());

        this.elements.calculateFinalBtn.addEventListener('click', () => this.calculateFinalPrice());

        // Percentage mode events
        this.elements.priceBeforeInput.addEventListener('input', () => this.validatePriceBefore());
        this.elements.priceAfterInput.addEventListener('input', () => this.validatePriceAfter());
        this.elements.calculatePercentageBtn.addEventListener('click', () => this.calculatePercentage());
    ...
    updateLivePreview() {
        ...
        if (!isNaN(originalPrice) && originalPrice > 0 && !isNaN(discountPercentage)) {
    ...
    validatePrice(input, errorElement) {
        ...
        if (input.value.trim() === '') { // Allow empty input on blur if not calculating
            this.hideError(errorElement);
            input.classList.remove('error');
            return true;
        } else if (!isValid) {
            this.showError(errorElement, 'Inserisci un prezzo valido e maggiore di zero.');
    ...
    validatePriceBefore() {
        return this.validatePrice(this.elements.priceBeforeInput, this.elements.priceBeforeError);
    }
    ...
    validatePriceAfter() {
        ...
        if (!isValidPriceAfter) {
            this.showError(this.elements.priceAfterError, 'Inserisci un prezzo valido.');
            this.elements.priceAfterInput.classList.add('error');
            return false;
        }

        if (!isNaN(priceBefore) && priceBefore > 0 && priceAfter >= priceBefore) {
            this.showError(this.elements.priceAfterError, 'Il prezzo scontato deve essere inferiore al prezzo originale.');
            this.elements.priceAfterInput.classList.add('error');
            return false;
        }
    ...
    async calculateFinalPrice() {
        ...
        const isPriceValid = this.validatePrice(this.elements.originalPriceInput, this.elements.priceError);
        ...
        this.showResult({
            ...
            details: [
                { label: 'Prezzo Originale', value: this.formatCurrency(originalPrice) },
                { label: 'Sconto', value: `${discountPercentage.toFixed(2)}%` }, // Display with 2 decimal places
                { label: 'Importo Risparmiato', value: this.formatCurrency(discountAmount) }
            ]
        });
    ...
    async calculatePercentage() {
        ...
        if (!isPriceBeforeValid || !isPriceAfterValid) {
            if (!isPriceBeforeValid) this.shakeElement(this.elements.priceBeforeInput);
    ...
    hideResults() {
        ...
        if (this.currentMode === 'final') {
            this.elements.finalPriceMode.style.display = 'block';
        } else {
            this.elements.percentageMode.style.display = 'block';
        }
    }
        ...
        if (navigator.share) {
            ...
            if (navigator.clipboard && navigator.clipboard.writeText) {
                navigator.clipboard.writeText(fullText)
                    .then(() => alert('Risultato copiato negli appunti! Puoi incollarlo dove vuoi.'))
                    .catch(err => console.error('Could not copy text: ', err));
            } else {
                alert('La funzione di condivisione non è supportata dal tuo browser. Puoi copiare il testo manualmente:\n\n' + fullText);
            }
    ...
    clearErrors() {
        ...
        document.querySelectorAll('.error-message').forEach(errorEl => this.hideError(errorEl));
        // Remove error class from all input fields
        document.querySelectorAll('input[type="number"]').forEach(inputEl => inputEl.classList.remove('error'));
    }
