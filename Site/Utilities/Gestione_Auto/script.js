...
function checkMaintenanceDue(maintenance, currentKm) {
  // Definizione della soglia per l'alert KM (0 km di preavviso)
  const KM_ALERT_THRESHOLD = 0;
...
function renderStats() {
  ...
  const completedMaintenances = maintenances.filter((m) => m.completed).length;
  completedMaintenancesEl.textContent = completedMaintenances;

  // Aggiorna le etichette per singolare/plurale
  totalVehiclesLabelEl.textContent =
    vehicles.length === 1 ? "Veicolo Totale" : "Veicoli Totali";
  totalMaintenancesLabelEl.textContent =
    maintenances.length === 1 ? "Manutenzione" : "Manutenzioni";
  activeAlertsLabelEl.textContent =
    alerts.length === 1 ? "Scadenza Attiva" : "Scadenze Attive";
  completedMaintenancesLabelEl.textContent =
    ...
    vehicleCard.innerHTML = `
            <div class="vehicle-card-header">
                ...
                <div class="vehicle-header-actions">
                    <button class="btn-icon btn-secondary" onclick="handleEditVehicle('${
                      vehicle.id
                    }')" aria-label="Modifica Veicolo">
                        <i class="fas fa-pencil-alt"></i>
                    </button>
                    <button class="btn-icon btn-danger" onclick="handleDeleteVehicle('${
                      vehicle.id
            ...
            <div class="vehicle-card-footer">
                <div class="vehicle-card-info-item">
                    ...
                    <span>${
                      vehicle.currentKm ? vehicle.currentKm.toLocaleString() : 0
                    } km</span>
                </div>
                <div class="vehicle-card-info-item">
            ...
            <div class="vehicle-card-actions">
                <button class="btn btn-secondary btn-full" onclick="handleViewDetails('${
                  vehicle.id
                }')">
                    Dettagli
                </button>
                <button class="btn btn-primary btn-full" onclick="handleAddMaintenance('${
                  vehicle.id
                }')">
...
function handleViewDetails(vehicleId) {
  ...
  if (vehicleMaintenances.length === 0) {
    maintenancesListEl.innerHTML =
      '<div class="no-maintenances"><p>Nessuna manutenzione programmata</p></div>';
  } else {
    maintenancesListEl.innerHTML = "";
    vehicleMaintenances.forEach((maintenance, index) => {
      ...
      maintenanceItem.className = `maintenance-item ${
        maintenance.completed ? "completed" : check.isDue ? "due" : ""
      }`;
                ...
                <div class="maintenance-item-header">
                    <div class="maintenance-item-info">
                        <div class="maintenance-item-header-left">
                            <button class="maintenance-checkbox" onclick="handleToggleComplete('${
                              maintenance.id
                            }')">
                                <i class="fas ${
                        ...
                        <div class="maintenance-details">
                            <div class="maintenance-detail-item" style="color: var(--text-secondary)">
                                ...
                                ${
                                  !maintenance.completed
                                    ? `
                                    <span class="${
                                      isOverdue ? "text-overdue" : "text-normal"
                                    }">
                                        (${
                                          check.daysUntil >= 0
                                            ? `tra ${check.daysUntil} ${
                            ...
                            ${
                              maintenance.notes
                                ? `<p class="maintenance-notes" style="color: var(--text-secondary)">${maintenance.notes}</p>`
                                : ""
                            }
                            ${
                    ...
                    <div class="maintenance-actions"> <button class="btn-icon btn-secondary" onclick="handleEditMaintenance('${
                      maintenance.id
                    }')" aria-label="Modifica Manutenzione">
                            <i class="fas fa-pencil-alt"></i>
                        </button>
                        <button class="btn-icon btn-danger" onclick="handleDeleteMaintenance('${
                          maintenance.id
...
btnAddVehicleEl.addEventListener("click", () => {
  ...
  document.getElementById("dialogAddVehicleTitle").textContent =
...
document
  .getElementById("btnCancelAddMaintenance")
...
document
  .getElementById("btnCancelEditMaintenance")
  .addEventListener("click", () => {
...
document
  .getElementById("btnEditVehicleFromDetails")
  .addEventListener("click", () => {
...
document
  .getElementById("btnAddMaintenanceFromDetails")
  .addEventListener("click", () => {
    if (!currentVehicle) return;
...
document
  .getElementById("btnUpdateKm")
  .addEventListener("click", handleUpdateKm);
...
formAddMaintenanceEl.addEventListener("submit", (e) => {
  ...
  const maintenance = {
    ...
    dueKm: document.getElementById("dueKm").value
...
formEditMaintenanceEl.addEventListener("submit", (e) => {
  ...
  const maintenance = {
    ...
    dueKm: document.getElementById("editDueKm").value
    ...
    dialog.addEventListener("click", (e) => {
      if (e.target === dialog) {
        ...
        if (dialog === dialogConfirmEl) {
          document.getElementById("btnConfirmAction").style.display =
            "inline-flex";
          document.getElementById("btnCancelConfirm").textContent = "Annulla";
        }
