...
// Storage functions
const getVehicles = () =>
  localStorage.getItem(DB_KEYS.VEHICLES)
    ? JSON.parse(localStorage.getItem(DB_KEYS.VEHICLES))
    : [];
...
function checkMaintenanceDue(maintenance, currentKm) {
  ...
  if (maintenance.dueKm && currentKm) {
    ...
    if (kmUntil <= 1000 && kmUntil >= 0) {
      ...
      isDue = true;
      reason = reason
        ? `${reason} e superato di ${Math.abs(kmUntil)} km`
        : `Superato di ${Math.abs(kmUntil)} km`;
    }
  }
