...
function convert() {
    ...
    const toUnitName = conversions[currentCategory]?.units[toUnitVal]?.name || toUnitVal;
    result.textContent = `${formatted} ${toUnitName.includes('(') ? toUnitName.split('(')[1].replace(')', '') : toUnitName}`;

    // Show conversion path
    const fromUnitName = conversions[currentCategory]?.units[fromUnitVal]?.name || fromUnitVal;
...
function convertTemperature(value, fromUnit, toUnit) {
    ...
    switch (fromUnit) {
        ...
        default: celsius = value; // Fallback, though ideally should not happen
    }
    ...
    switch (toUnit) {
        ...
        default: return celsius; // Fallback
    }
}
...
function updateQuickConversions() {
    ...
    if (!conversions[currentCategory]) {
        quickConversions.innerHTML = '<div class="text-center text-gray-500 py-8">Caricamento conversioni rapide...</div>';
        return;
    }
