...
if (textInput) {
  textInput.addEventListener("input", () => {
    ...
    const wordCount =
      text.trim() === "" ? 0 : (text.match(/\b\w+\b/g) || []).length;
    ...
    animateNumber(
      sentenceCountEl,
      parseInt(sentenceCountEl.textContent),
      sentenceCount,
    );
...
function analyzeGrammar() {
  ...
  setTimeout(() => {
    ...
    const analysis = words.map((word) => {
      if (word.match(/are$/))
        return `<span style="color: #10b981; font-weight: 600;">${word}</span> <small>(verbo-are)</small>`;
      if (word.match(/ere$/))
        return `<span style="color: #10b981; font-weight: 600;">${word}</span> <small>(verbo-ere)</small>`;
      if (word.match(/ire$/))
        return `<span style="color: #10b981; font-weight: 600;">${word}</span> <small>(verbo-ire)</small>`;
      if (word.match(/mente$/))
        return `<span style="color: #f59e0b; font-weight: 600;">${word}</span> <small>(avverbio)</small>`;
      if (word.match(/^(il|la|lo|gli|le|un|una|uno)$/))
        return `<span style="color: #ec4899; font-weight: 600;">${word}</span> <small>(articolo)</small>`;
      if (word.match(/^(e|o|ma|però|quindi)$/))
        return `<span style="color: #8b5cf6; font-weight: 600;">${word}</span> <small>(congiunzione)</small>`;
      if (word.match(/i$/))
        return `<span style="color: #06b6d4; font-weight: 600;">${word}</span> <small>(nome-plurale)</small>`;
...
function startTypingTest() {
  ...
  typingInterval = setInterval(() => {
    ...
    const words =
      input.value.trim() === ""
        ? 0
        : (input.value.match(/\b\w+\b/g) || []).length;
