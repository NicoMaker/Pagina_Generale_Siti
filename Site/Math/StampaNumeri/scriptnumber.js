function generaTabella() {
  ...
  if (isNaN(numero) || numero <= 0) {
    risultato.innerHTML = '<p class="error-message">Inserisci un numero valido maggiore di zero.</p>';
  ...
  if (scelta === "tutti") {
    ...
    numeri = Array.from({ length: Math.ceil(numero / 2) }, (_, i) => (i + 1) * 2)
      .filter(num => num <= numero);
  } else if (scelta === "dispari") {
    numeri = Array.from({ length: Math.ceil(numero / 2) }, (_, i) => i * 2 + 1)
      .filter(num => num <= numero);
  }
  ...
  for (let i = 0; i < numeri.length; i++) {
    ...
    if ((i % columnsPerRow === columnsPerRow - 1) || (i === numeri.length - 1)) tabella += "</tr>";
  }
  tabella += "</table>";
