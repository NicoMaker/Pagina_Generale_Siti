...
function drawIntroTriangle() {
    ...
    const hx =
        ax + ((bx - ax) * ((cx - ax) * (bx - ax) + (cy - ay) * (by - ay))) / (Math.pow(bx - ax, 2) + Math.pow(by - ay, 2))
    const hy =
        ay + ((by - ay) * ((cx - ax) * (bx - ax) + (cy - ay) * (by - ay))) / (Math.pow(bx - ax, 2) + Math.pow(by - ay, 2))
...
function drawFirstTheoremTriangle(canvas, a, b, c, p, q) {
    ...
    ctx.beginPath()
    ctx.moveTo(cx, cy)
    ctx.lineTo(cx - squareSize * Math.cos(angle - Math.PI / 2), cy + squareSize * Math.sin(angle - Math.PI / 2))
    ctx.lineTo(
        cx - squareSize * Math.cos(angle - Math.PI / 2) + squareSize * Math.cos(angle),
        cy + squareSize * Math.sin(angle - Math.PI / 2) - squareSize * Math.sin(angle),
    )
    ctx.strokeStyle = "#A45A52"
    ctx.stroke()
    ...
    ctx.fill()
    ctx.fillStyle = "#2C3E50"
    ctx.fillText(`a² = ${a * a}`, 50 + (a * scale) / 2 - 30, 50 + (a * scale) / 2)
    ...
    ctx.fill()
    ctx.fillStyle = "#2C3E50"
    ctx.fillText(`c·p = ${(c * p).toFixed(2)}`, 50 + (c * scale) / 2 - 40, 50 + a * scale + 20 + (p * scale) / 2)
...
function drawSecondTheoremTriangle(canvas, a, b, c, p, q, h) {
    ...
    ctx.fill()
    ctx.fillStyle = "#2C3E50"
    ctx.fillText(`h² = ${(h * h).toFixed(2)}`, 50 + (h * scale) / 2 - 30, 50 + (h * scale) / 2)
    ...
    ctx.fill()
    ctx.fillStyle = "#2C3E50"
    ctx.fillText(`p·q = ${(p * q).toFixed(2)}`, 50 + (p * scale) / 2 - 30, 50 + h * scale + 20 + (q * scale) / 2)
...
function checkAnswer(userAnswer, correctAnswer, feedbackElement) {
    ...
    if (!isNaN(userAnswer) && Math.abs(userAnswer - correctAnswer) < tolerance) {
        ...
        feedbackElement.textContent = "Risposta errata. Riprova o consulta la soluzione."
