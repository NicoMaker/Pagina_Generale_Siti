...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Carica i dati salvati della sessione corrente
    ...
    // Popola il dropdown delle sessioni
    updateSessionsDropdown();
});

// Carica tutte le sessioni salvate
function loadSessions() {
    const savedSessions = localStorage.getItem('turnManagementSessions');
    if (savedSessions) {
        try {
    ...
    // Se non ci sono sessioni, crea quella di default
...
// Salva tutte le sessioni nel localStorage
function saveSessionsToLocalStorage() {
    localStorage.setItem('turnManagementSessions', JSON.stringify(sessions));
}
...
function saveCurrentSession() {
    const sessionName = sessionNameInput.value.trim() || 'Sessione ' + new Date().toLocaleString();
    ...
    showNotification(`Sessione "${sessionName}" salvata con successo!`);
    playSound('complete');
...
// Carica una sessione selezionata
function loadSelectedSession() {
    const selectedSession = sessionsDropdown.value;
    
    if (sessions[selectedSession]) {
        currentSessionName = selectedSession;
        const session = sessions[selectedSession];
        
        people = [...session.people];
        currentTurn = session.currentTurn;
        currentPersonIndex = session.currentPersonIndex;
        ...
        showNotification(`Sessione "${selectedSession}" caricata con successo!`);
        playSound('start');
    }
}

// Elimina una sessione selezionata
function deleteSelectedSession() {
    const selectedSession = sessionsDropdown.value;
    
    if (selectedSession !== 'default' && sessions[selectedSession]) {
        delete sessions[selectedSession];
        saveSessionsToLocalStorage();
        
        // Se la sessione corrente è stata eliminata, carica quella di default
        if (selectedSession === currentSessionName) {
            currentSessionName = 'default';
            loadFromLocalStorage();
        }
        
        updateSessionsDropdown();
        showNotification(`Sessione "${selectedSession}" eliminata!`);
        playSound('delete');
    } else if (selectedSession === 'default') {
        showNotification("Non puoi eliminare la sessione predefinita!");
        playSound('error');
    }
}

// Esporta tutte le sessioni in un file JSON
function exportData() {
    // Aggiorna la sessione corrente prima dell'esportazione
    ...
    const exportFileDefaultName = 'gestione_turni_' + new Date().toISOString().slice(0, 10) + '.json';
...
// Importa sessioni da un file JSON
function importData() {
    fileInput.click();
}

// Gestisce l'importazione del file
function handleFileImport(event) {
    ...
    reader.onload = function(e) {
        try {
            ...
            if (typeof importedSessions === 'object') {
                // Unisci le sessioni importate con quelle esistenti
...
function createAddPersonInTurnSection() {
    ...
    turnNameInput.addEventListener('keypress', function (e) {
        if (e.key === 'Enter') {
...
function loadFromLocalStorage() {
    ...
    if (sessions[currentSessionName]) {
        const session = sessions[currentSessionName];
        people = [...session.people];
        currentTurn = session.currentTurn;
        currentPersonIndex = session.currentPersonIndex;
        isGameStarted = session.isGameStarted;
