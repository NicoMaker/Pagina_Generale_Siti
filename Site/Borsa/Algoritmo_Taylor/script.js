document.addEventListener("DOMContentLoaded", function () {
    ...
    if (typeof math === 'undefined') {
        ...
        alert('Errore: math.js non è stato caricato. Ricarica la pagina o controlla la console per dettagli.');
        return;
    }
            ...
            data: {
                datasets: [
                    ...
                    },
                    {
                        ...
                        borderColor: getComputedStyle(document.documentElement).getPropertyValue("--approximation-color"),
                        borderWidth: 2,
                        pointRadius: 0,
                        fill: false,
                        borderDash: [5, 5]
    ...
    function updateChart() {
        ...
        const centerPoint = parseFloat(document.getElementById("center-point").value);
        ...
        try {
            ...
            // Show error message
            errorElement.textContent = "Errore: espressione non valida. Prova con 'sin(x)', 'x^2', ecc.";
        ...
        for (let x = xMin; x <= xMax; x += step) {
            ...
            try {
                const taylorY = evaluateTaylorSeries(functionExpression, x, centerPoint, terms);
        ...
        const expansionHTML = generateExpansionHTML(functionExpression, centerPoint, terms);
    ...
    function evaluateTaylorSeries(expression, x, a, terms) {
        ...
        } else if (lowerExpression === "sqrt(1+x)" || lowerExpression === "sqrt(1 + x)") {
    ...
    function numericalDerivative(expression, x, order, h = 0.0001) {
        ...
        // Higher order derivatives using recursion
        return (numericalDerivative(expression, x + h, order - 1, h) -
            numericalDerivative(expression, x - h, order - 1, h)) / (2 * h);
    }
    ...
    document.getElementById("calculate-btn").addEventListener("click", function () {
        ...
        const centerPoint = parseFloat(document.getElementById("calc-center").value);
        ...
        const approximation = evaluateTaylorSeries(functionExpression, evalPoint, centerPoint, terms);
        ...
        // Display results
        document.getElementById("exact-result").textContent = exactValue.toFixed(8);
        document.getElementById("approximation-result").textContent = approximation.toFixed(8);
        document.getElementById("error-result").textContent = absoluteError.toFixed(8);
        document.getElementById("relative-error-result").textContent = relativeError.toFixed(4) + "%";
