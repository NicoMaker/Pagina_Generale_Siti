document.addEventListener("DOMContentLoaded", () => {
  ...
  let nodes = [];
  let connections = [];
  const animationSpeed = 0.8; // Velocità di animazione aumentata (era 0.3)

  // Initialize visualization
  function initializeVisualization() {
    const duration = Number.parseInt(elementsSlider.value);
    ...
    connections = [];
    const rateChange = Number.parseFloat(complexitySlider.value);
  ...
  function updateVisualization() {
    ...
    const duration = Number.parseInt(elementsSlider.value);
    const rateChange = Number.parseFloat(complexitySlider.value);
    const convexity = Number.parseFloat(recursionSlider.value);
    ...
    const effectiveness = Math.min(
      0.99,
      Math.max(0.01, (durationMatch + convexityProtection) * rateImpact),
    );

    // Animazione più veloce del valore di efficacia
    animateValue(
      predictabilityValue,
      Number.parseFloat(predictabilityValue.textContent),
      effectiveness,
      500,
    ); // Ridotto da 1000ms a 500ms
  ...
  function drawYieldCurve() {
    ...
    // Create gradient for the curve - colori moderni finanziari
    const gradient = ctx.createLinearGradient(0, 0, canvas.offsetWidth, 0);
    gradient.addColorStop(0, "rgba(16, 185, 129, 0.8)"); // Verde
    gradient.addColorStop(0.5, "rgba(6, 182, 212, 0.8)"); // Teal
    gradient.addColorStop(1, "rgba(234, 179, 8, 0.8)"); // Oro
    ...
    for (let x = 0; x < canvas.offsetWidth; x += 2) {
      ...
      const rate =
        baseRate +
        steepness * Math.log(1 + maturity) -
        (curvature * Math.pow(maturity - 10, 2)) / 100;
  ...
  function animateNodes() {
    ...
    nodes.forEach((node) => {
      ...
      if (node.x > canvas.offsetWidth - node.radius)
        node.x = canvas.offsetWidth - node.radius;
      if (node.y < node.radius) node.y = node.radius;
      if (node.y > canvas.offsetHeight - node.radius)
